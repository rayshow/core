	...
	namespace mpl
	{
		...
		namespace fn_detail
		{
			...
			struct dispatch_t
			{
				...
				constexpr dispatch_t(Fn&& inFn, Fs&&... fs)
					: fn{ util::move(inFn) }, storage{ util::move(fs)... }
				{}

				constexpr dispatch_t(Fn const& inFn, Fs const&... fs)
					: fn(inFn), storage{ fs... }
				{}


				template<typename T, typename... Args, uint32... indice>
				constexpr auto __invoke_impl(indice_pack<indice...>, T&& t, Args&&... args) const
				{
				...
				{
					static_assert(sizeof...(Args)+1 == L, "functions and parameters not match");
