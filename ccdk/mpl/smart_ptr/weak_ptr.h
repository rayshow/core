...
public:
	...
	/* copy from share_ptr<T2,D2,R2>, inc ref count */
	...
	/* copy from weak_ptr<T2,D2,R2>, inc ref count */
	...
	/* copy assign from share_ptr<T2,D2,R2> */
	template< typename T2, typename D2, typename R2, typename = check_t< is_convertible< share_ptr<T2, D2, R2>, share_type > > >
	...
	/* lock  */
	CCDK_FORCEINLINE share_type lock() const  { if (ptr) { return share_type{ *this }; } ccdk_throw(bad_weak_ptr{}); }
