...
>
class forward_list : protected Alloc::rebind<Node>
{
public:
...
public:
	...
	/* default */
	CCDK_FORCEINLINE constexpr forward_list() noexcept :head{nullptr},len{0}{¡¿}

	/* fill */
	CCDK_FORCEINLINE forward_list(size_type n, T const& t = T()) {
		allocate_fill(n, t);
	}

	/* copy range-n */
	template<typename InputIt, typename = check_t< is_iterator<InputIt>>>
	CCDK_FORCEINLINE forward_list(InputIt beginIt, size_type n) {
		allocate_copy(beginIt, n);
	}

	/* copy range */
	template<typename InputIt, typename = check_t< is_iterator<InputIt>>>
	CCDK_FORCEINLINE forward_list(InputIt beginIt, InputIt endIt) {
		allocate_copy(beginIt, alg::distance(beginIt, endIt));
	}

	/* template copy */
	template<typename Size2, typename Alloc2, typename Node2>
	CCDK_FORCEINLINE forward_list(forward_list<T,Size2,Alloc2,Node2> const& other) {
		allocate_copy(other.begin(), other,len);
	}

	/* copy */
	CCDK_FORCEINLINE forward_list(forward_list const& other) {
		allocate_copy(other.begin(), other, len);
	}

	/* template move */
	template<typename Size2, typename Alloc2, typename Node2>
	CCDK_FORCEINLINE forward_list(forward_list<T, Size2, Alloc, Node> && other)
		noexcept : head{ other.head }, len{ other.len } {
		other.rvalue_reset();
	}

	/* move */
	CCDK_FORCEINLINE forward_list(forward_list && other)
		noexcept : head{ other.head }, len{ other.len } {
		other.rvalue_reset();
	}

	CCDK_FORCEINLINE void swap(forward_list & other) noexcept {
	...
	template<typename Size2, typename Alloc2, typename Node2>
	CCDK_FORCEINLINE this_type& operator=(forward_list<T, Size2, Alloc2, Node2> const& other) {
		assign_copy(other.begin(), other.len);
	}

	/* copy assign */
	CCDK_FORCEINLINE this_type& operator=(forward_list const& other) {
		assign_copy(other.begin(), other.len);
	}

	/* template copy assign  */
	template<typename Size2, typename Alloc2, typename Node2>
	CCDK_FORCEINLINE this_type& operator=(forward_list<T, Size2, Alloc, Node> && other) {
		rvalue_set(other.head, other.len);
		other.rvalue_reset();
	}

	/* move assign */
	CCDK_FORCEINLINE this_type& operator=(forward_list && other) {
		rvalue_set(other.head, other.len);
		other.rvalue_reset();
	}
