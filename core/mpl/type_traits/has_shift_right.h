...
namespace core
{
	namespace mpl
	{
		...
		HAS_BINARY_OPERATION_DECL(>> , shift_right, A3D_PP_BIT_OP_NOT_MATCH(left_nocv_t, right_nocv_t));

		// const T >>= P is ill-formed 
		HAS_BINARY_OPERATION_DECL(>>=, shift_right_assign, A3D_PP_BIT_OP_NOT_MATCH(left_nocv_t, right_nocv_t) || is_const_v<left>);
	}
}